...
print("The eigenvalues solved from the effective two-particle Hamiltonian: ")
print(effVals.real)
end = time.time()
...
imag_iw = np.zeros((niw,2*occ*virt,2*occ*virt),dtype=np.complex128)
for iw in range(niw):
    imag_iw[iw,:] += np.eye(2*occ*virt) * wgrid[iw] * 1j
F2p_Dyn -= imag_iw


F2p_Dyn_inv = np.zeros(H2p_Dyn.shape,dtype=np.complex128)
for iw in range(niw):
    F2p_Dyn_inv[iw,:] += LA.inv(F2p_Dyn[iw,:])


F2p_Dyn_inv_tau = ct.omega2tauFT(F2p_Dyn_inv,beta,ir_file)


f = h5py.File(out_path, 'w')
f["/ResFunc_tau/data"]  = F2p_Dyn_inv_tau
f["/ResFunc_iw/data"]   = F2p_Dyn_inv
